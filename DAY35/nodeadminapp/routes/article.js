...
router.post('/create',isLoggedIn,simpleUpload.single('file1'), async(req, res, next)=> {
...
//upload.getUpload('upload/').fields([{ name: 'client파일태그명', maxCount: 1 }]) > 맥스카운트는 최대 몇개까지 업로드 가능한지 처리
router.post('/creates3',isLoggedIn,upload.getUpload('/').fields([{ name: 'file', maxCount: 1 }]), async(req, res, next)=> {

...
router.post('/passportLogin',async(req,res,next)=>{
  ...
  //passport.authenticate('로그인전략=local',패스포트 로그인후 수행되는 콜백함수=done('패스포트 실행결과-정상,에러,세션데이터,추가메시지'))
  passport.authenticate('local',(authError,admin,info)=>{

    //패스포트 인증시 에러가 발생한 경우 에러값이 반환됨.
    if(authError){
      console.log(authError);
      return next(authError);
    }

    //로컬 전략 파일에서 전달된 관리자 세션데이터가 전달이 안된 경우..
    //동일 아이디화 암호가 없는 경우 done('',false)두번째 파라메터의 값이 false로 전달됨..
    //아이디 암호가 틀린 경우 체크
    if(!admin){

      //flash패키지 설치 필요: flash패키지는 서버 기반에서 특정 페이지 이동시 바로 전에 특정데이터를 전달해주고 싶을 때 사용
      //req.flash('키명',키값)
    ...
    //req.login('세션으로 저장할 사용자 데이터', 처리결과 콜백함수(login실행시 발생한 에러값))은 passport기반 정상 인증이 완료되면 passport세션을 생성해주는 기능 제공

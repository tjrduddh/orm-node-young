...
VALUES('test1@test.co.kr','1234','오영석','','010-3182-1101',1,1,'940123',now(),1);

INSERT INTO member(email,member_password,name,profile_img_path,telephone,entry_type_code,use_state_code,birth_date,reg_date,reg_member_id)
VALUES('test2@test.co.kr','1234','오영석2','','010-3182-1401',1,1,'940623',now(),1);

INSERT INTO member(email,member_password,name,profile_img_path,telephone,entry_type_code,use_state_code,birth_date,reg_date,reg_member_id)
VALUES('test3@test.co.kr','1234','오영석3','','010-3182-1501',1,1,'910923',now(),1);

INSERT INTO member(email,member_password,name,profile_img_path,telephone,entry_type_code,use_state_code,birth_date,reg_date,reg_member_id)
VALUES('test4@test.co.kr','1234','오영석4','','010-3382-1501',1,1,'910913',now(),1);


#READ DATA - 데이터 조회 / SELECT 구문
SELECT * FROM member;
SELECT * FROM member WHERE email = 'test1@test.co.kr'
SELECT * FROM member WHERE entry_type_code = 1 AND name = '오영석';
SELECT * FROM member WHERE entry_type_code = 1 OR use_state_code = 0;
SELECT * FROM member WHERE member_id <= 3;
SELECT * FROM member WHERE name IN('오영석2','오영석3','오영석4');
SELECT * FROM member WHERE name LIKE '%오%'; #패턴매칭  %가 가로 시작하는 모든 항목, %가% 가가 포함된 모든 데이터 가%가로 끝나는 모든 데이터

SELECT * FROM member ORDER BY member_id DESC;
SELECT * FROM member ORDER BY member_id ASC;

#UPDATE DATA - 데이터 수정 / UPDATE 구문
UPDATE member SET name = '오영석0',profile_img_path='http://naver.com/images/test.png' WHERE member_id=1;

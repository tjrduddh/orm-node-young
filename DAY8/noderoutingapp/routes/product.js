...
//localhost:3000/product/detail?pid=1&pname=lg노트북&price=6000
router.get('/detail',async(req,res)=>{
    ...
    //단일객체 변수를 할당하게 됬는데 속성명과 이름이 같아 한개로 생략했다
...
//호출주소체계: localhost:3000/product/detail/smaple
//호출방식: GET
router.get('/detail/sample',async(req,res)=>{
    res.render('product/detail',{productID:100,productName:"노트북",price:"6000"});
});
...
router.get('/detail/sendall',async(req,res)=>{
    ...
    // //json데이터를 전송해보자
    // res.send({uid:"eddy",username:"오영석"});
...
//호출방식: GET 유형
//반환값: 단일상품정보 웹페이지
//*매우중요* : 와일드카드 방식으로 주소체계가 정의된 라우팅메소드는 해당 라우터 파일의 맨 하단에 배치시켜야한다
router.get('/detail/:pid',async(req,res)=>{

    //URL을 통해 파라메터 방식으로 값이 전달되면
    //주소체계내에 와일드카드 키를 설정하고 해당 키명으로 URL을 통해 전달되 파라메터 값을
    //추출(req.params.와일드카드키명) 할 수 있다
    var productID = req.params.pid;

    res.render('product/detail',{productID,productName:"노트북",price:"6000"});
}); 


//호출주소: http://localhost:3000/product/detail/1/LG노트북/6000
//호출방식: GET
//여러개의 파라메터를 여러개의 와일드카드 키명으로 호출해서

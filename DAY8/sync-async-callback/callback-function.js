...
//콜백함수 방식으로 기능구현하기
//기존 logging 함수를 특정함수에 파라메터로 전달해서 해당 함수내에서 전달된 함수를 실행하는 방식..콜백함수
...
//일반화: 클래스와 객체(실체=Object,Entitiy): 실체(현실에 존재하는 물체)의 공통 속성/기능을 일반화시킨 개념이..클래스
//추상화,상속,다형성,캡슐화

//추상화(asbstraction): 상속(부모클래스를 상속받아 자식클래스를 만드는 것)과 인터페이스
//추상화 인터페이스(미리 클래스에서 구현해야하는 기능과 속성 정의만 한다-구현은 하지않는다)

...
router.post('/maketoken', async (req, res, next) => {
  ...
  //JWT토큰에 담을 JSON데이터 구조 및 데이터 바인딩
  //JWT토큰 영역내 Payload영역에 담깁니다.
  var jsonTokenData = {
  ...
  // jwt.sign(토큰에 담을 JSON데이터,토큰인증키,옵션{expiresIn:'24h'파기일 지정,issuer:'msoftware'만든회사})
  //파기일시 지정 포맷 : 30s,60m,24h,200d  , 토큰생성일을 기준으로 해서 해당기간만큼만 유효함,
...
router.get('/readtoken', async (req, res, next) => {
  ...
  try{
    ...
    //await jwt.verify(JWT토큰문자열,해당 토큰생성시 사용한 JWT인증키 값);

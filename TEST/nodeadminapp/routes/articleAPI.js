...
var storage = multer.diskStorage({
    ...
    filename(req, file, cb) {
        ...
        //여기서 올리는 시간을 적어주는 이유는 파일명이 중복되면 덮어쓰기가 됨으로 방지를 위해서 시간을 적어줍니다.
...
router.post('/upload',upload.single('file'),async(req,res)=>{
    ...
    try{
        ...
        var fileName = uploadFile.filename; // ㅅㅓ버에 저장된 실제 물리파일명(파일명/확장자포함)
        var fileOrignalName = uploadFile.originalname; // 사용자가 직접 작성한 파일명
        var fileSize = uploadFile.size;  // 파일크기 KB
        var fileType = uploadFile.mimetype;  // 파일 포맷



        //정상 데이터 등록처리 결과값 세팅하기 
        apiResult.code = 200;
